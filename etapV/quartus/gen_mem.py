...
def build_memory(hosts):
    ...
    # 1. Budowa Filtra Blooma
    for flow in hosts:
        flow_128 = flow_to_128bit(flow)
        for i in range(BLOOM_HASH_COUNT):
            idx = hw_hash(flow_128, 17, SEEDS_BLOOM[i])
            bloom_array[idx] = 1

    # 2. Budowa Cuckoo Hashing
    for flow in hosts:
        ...
        for _ in range(MAX_KICKS):
            ...
            for bank_idx in range(CUCKOO_BANKS):
                addr = hw_hash(current_flow_128, 13, SEEDS_CUCKOO[bank_idx])
            ...
            kick_addr = hw_hash(current_flow_128, 13, SEEDS_CUCKOO[kick_bank])
        ...
        if not inserted:
            raise Exception(f"Błąd krytyczny: Pętla nieskończona Cuckoo dla hosta {flow}")
